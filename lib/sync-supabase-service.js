...
async function overwriteOperationalData({ children, records, ifMatchRev, actor, deviceId, appVersion }) {
  ...
  (existingChildrenRows || []).forEach(row => {
    ...
    if (row.id && !isEmptyEnrollmentHistory(existingPayload.enrollmentHistory)) {
      historyById.set(row.id, existingPayload.enrollmentHistory);
    }
  });

  const mergedChildren = children.map(child => {
    if (!child?.id) return child;
    if (!isEmptyEnrollmentHistory(child.enrollmentHistory)) return child;
    if (!historyById.has(child.id)) return child;
    return { ...child, enrollmentHistory: historyById.get(child.id) };
  });
