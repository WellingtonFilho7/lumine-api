...
function loadHandler({
  ...
  const handlerPath = require.resolve('../../api/admin/operational-backup/download');
  ...
  const handler = require('../../api/admin/operational-backup/download');
...
test('admin operational backup download responde com attachment json', async () => {
  ...
  try {
    ...
    await handler({ method: 'GET', headers: { 'x-user-jwt': 'jwt' } }, res);
