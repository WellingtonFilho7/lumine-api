...
function normalizeCreatePayload(raw) {
  ...
  payload.categoria = sanitizeText(payload.categoria, 120);
...
function normalizeListQuery(raw) {
  ...
  query.categoria = sanitizeOptional(query.categoria, 120);
  query.startDate = sanitizeOptional(query.startDate || query.start || query.dataInicio, 20);
  query.endDate = sanitizeOptional(query.endDate || query.end || query.dataFim, 20);
...
const financeUploadUrlSchema = z.preprocess(
  ...
  z.object({
    ...
    contentType: z
      ...
      .refine(value => ALLOWED_UPLOAD_MIME.includes(value), 'contentType invalido'),
